...
from math import atan2, cos, prod, sin, sqrt
...
class Corpo(Punto):
    ...
    def __init__(self, posizione: list = [0, 0, 0], carica: float = 0):
...
def polari_cartesiane(coordinate_polari: list[float]) -> list[float]:
    ...
    while pol:
        ...
        if pol:  # Se ci sono ancora angoli nella lista
            seni = [
                float(sin(x)) for x in pol
            ]  # Calcola il seno di tutti gli angoli rimanenti
            produttoria = prod(seni)  # Moltiplica tutti i seni tra loro
            cartesiane.append(
                r * coseno * produttoria
            )  # Aggiungi il valore alla lista cartesiana
        else:
            cartesiane.append(
                r * coseno
            )  # Se non ci sono più angoli, aggiungi solo r*coseno
